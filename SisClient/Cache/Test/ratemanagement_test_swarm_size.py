...
def test_ratemanagement_swarm_size(conf):
    
    if not os.path.exists("cache_tests/ratemanagement_test/"):
        os.makedirs("cache_tests/ratemanagement_test/")
    
    conf.set_base_directory("cache_tests/ratemanagement_test/swarm_size/")
    conf.set_test_name("Ratemanagement test (swarm_size)")
    ...
    set_peers(conf, "cache_tests/ratemanagement_test/swarm_size/", "SisClient/Cache/Test/config/config1.cfg")
