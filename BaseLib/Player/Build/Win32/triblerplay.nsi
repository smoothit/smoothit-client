!define PRODUCT "SmoothIT Player"
...
; Remove old error log if present
 Delete "$INSTDIR\swarmplayer-sis.exe.log"
...
Section "Desktop Icons" SecDesk
   CreateShortCut "$DESKTOP\${PRODUCT}.lnk" "$INSTDIR\swarmplayer-sis.exe" ""
SectionEnd
...
Section "Startmenu Icons" SecStart
   ...
   CreateShortCut "$SMPROGRAMS\${PRODUCT}\${PRODUCT}.lnk" "$INSTDIR\swarmplayer-sis.exe" "" "$INSTDIR\swarmplayer-sis.exe" 0
SectionEnd
...
Section "Make Default For .tstream" SecDefaultTStream
   ...
   WriteRegStr HKCR "tstream\shell\open\command" "" '"$INSTDIR\swarmplayer-sis.exe" "%1"'
...
;   WriteRegStr HKCR "bittorrent\shell\open\command" "" '"$INSTDIR\swarmplayer-sis.exe" "%1"'
...
Function .onInit
  System::Call 'kernel32::CreateMutexA(i 0, i 0, t "SmoothIT Player") i .r1 ?e' 
