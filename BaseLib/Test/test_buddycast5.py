...
class TestBuddyCast(TestAsServer):
    """ 
    Testing BuddyCast 5 protocol interactions:
      swarm size info exachange.
    """
...
#       self.utility = Utility(installdir)        
    ...
    def get_good_clicklog_msg(self, n):
        if n==1:
            preferences = [["hash1", ["linux","ubuntu"], 1, 2, 400, 500, 1000, 50]]
            collected_torrents = ["hash"]
        elif n==2:
            preferences = [["hash2", ["linux", "ubuntu"], 2, 2, 600, 700,20000,60]]
            collected_torrents = ["hash2"]            
        elif n==3:
            preferences = [["hash3", ["linux","redhat"],5 ,2 , 800, 900, 30000, 70]]
...
if __name__ == "__main__":
    ...
    print "test BC messageeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
    unittest.main()
   # subtest_good_buddycast_clicklog(1)
