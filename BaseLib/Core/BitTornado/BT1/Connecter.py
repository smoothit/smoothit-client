...
from BaseLib.Core.Status import Status
...
class Connection:
    ...
    def got_ut_pex(self,d):
        ...
        if len(sample_added_peers_with_id) > 0:
            ...
            # smoothIT_
            ipPortsToAsk = []
            for ((ip, port), nr) in sample_added_peers_with_id:
                ipPortsToAsk.append((ip, port))
            acceptedIPPorts = self.neighborselection.selectAsNeighbors(ipPortsToAsk) 
            peers = []  
            for ((ip, port), nr) in sample_added_peers_with_id:
                if (ip, port) in acceptedIPPorts :
                    peers.append(((ip, port), nr))     
            self.connection.Encoder.start_connections(peers)
            # _smoothIT
...
# 2fastbt_
    ...
    def got_hashpiece(self, connection, message):
        ...
        try:
			...
			#SmoothIT_ append connection related data (not required?)
