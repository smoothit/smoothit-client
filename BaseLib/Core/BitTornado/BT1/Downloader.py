...
# 2fastbt_
try:
    from BaseLib.Core.CoopDownload.Helper import SingleDownloadHelperInterface
except ImportError:
...
#        self.download_rate = 25000  # 25K/s test rate
    ...
    def send_support_required(self, required):                
        if self.support_required == required: return
        self.logger.warn("SEND SUPPORT REQUIRED %s with %d neighbors" % (required, len(self.downloads)))
        
        self.support_required = required
        for d in self.downloads:     
            d.support_required = self.support_required       
            if self.support_required:
                d.send_interested()
            else:
                d.send_not_interested()
        if len(self.downloads)==0:
            self.logger.warn("NO NEIGHBORS FOUND!!!")
    
    def set_supporter_ips(self, supporter_ips):
        self.supporter_ips = supporter_ips
        self.logger.warn("supporter ips = %s" % self.supporter_ips)    

    #SmoothIT_
    
    def fetch_block_stats(self):
        '''
        Fetch (and clear) block statistics.
        '''
        all_stats = []
        for d in self.downloads:
            c = d.connection
            for (i, o, l, t) in d.block_stats: # just block data and timestamp
                entry = (i, o, l, t, c.get_ip(), c.get_port(), c.get_id()) # append source data
                all_stats.append(entry)
            d.block_stats = [] # clear stats!
        return all_stats
    #_SmoothIT
    ...
    def make_download(self, connection):
        ...
        # smoothit_
        #TODO: this is not the remote listenPort, where can we get it from for Logging?
        #port = connection.get_port()
        #session = BaseLib.Core.Session.Session.get_instance()
        #listenPort = session.get_listen_port()
        #self.logger.debug(NAME + " DOWNLOADING FROM " + str((ip, port))+ ", OWN PORT: " + str(listenPort))
        # _smoothit
