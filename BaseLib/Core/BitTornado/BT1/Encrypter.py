...
#smoothIT_
import SisClient.PeerSelection.NeighborSelection
#_smoothIT
...
# _2fastbt        
        ...
        # SmoothIT_
        self.neighborselection = SisClient.PeerSelection.NeighborSelection.getInstance()
        # _SmoothIT
    ...
    def external_connection_made(self, connection):
        ...
        return True
        #SmoothIT_ (separation inserted, has to be uncommented for real scenario)
        # TODO: this isn't the listen port of the remote peer - where can we get it from??
        #if self.neighborselection.acceptAsNeighbor((connection.get_ip(), connection.get_port(True)), connection.get_myport(True)):
        #    con = Connection(self, connection, None)
        #    self.connections[connection] = con
        #    connection.set_handler(con)
        #    return True
    ...
    def externally_handshaked_connection_made(self, connection, options, msg_remainder):
        ...
        #SmoothIT_ (separation inserted, has to be uncommented for real scenario)
        # TODO: this isn't the listen port of the remote peer - where can we get it from??
        #if self.neighborselection.acceptAsNeighbor((connection.get_ip(), connection.get_port(False))):
