...
# smoothit_
from SisClient.Tracker.TrackerStatistics import TrackerStatistics
import SisClient.TrackerExt.SupporterMonitor as SM
import logging
# _smoothit
...
class Tracker:
    def __init__(self, config, rawserver):
        # smoothit_
        self._statistics = TrackerStatistics(self)
        self._monitor = SM.SupporterMonitor()
        self._info = {}
        self._logger = logging.getLogger("Tracker")
        # _smoothit
    ...
    def get_infopage(self):
        try:
            ...
            s.write('<hr>\n')
            s.write(self.dump_becache_to_html())
            s.write('<hr>\n')

            s.write(SM.MonitorState.retrieve_monitor_state_as_html(self._monitor))
    ...
    def dump_becache_to_console(self):
        '''Dumps the becache contents to the stderr. The format of the dump contains:
        TRACKER DUMP, timestamp, infohash, nr_seeds, nr_leechers, list of peers: (peer_id, port, addr, type)
        (separated by \t on stdout). The dump occurs every time the tracker has handled
        add_data, which means every time it has just updated its internal state
        immediately before the dump.
        
        Example (for a single swarm):
        TRACKER DUMP    1242842.34    Locality-Demo.mp4    1    1
        N240-----nfiMC.6EwyB    127.0.0.1:5000  LEECHER
        N240-----dwtwpRRIifC    127.0.0.1:9000  SEEDER
        TRACKER DUMP    1242843.45    file.wmv             0    1
        N240-----twjklfsKDkc    127.0.0.1:6000  SEEDER
        '''
        if not self.becache:
            return
        ...
        for infohash in bc.keys():
            swarm = bc[infohash]
            
            print >>sys.stderr, 'TRACKER DUMP\t%s\t%s\t%i\t%i' % (str(time()),
                                                                  self.allowed[infohash]['name'],
                                                                  len(swarm[0][0].keys()),
                                                                  len(swarm[0][1].keys()))
            for leecher in swarm[0][0].keys():
                r = bdecode(swarm[0][0][leecher].bencoded)
                addr = '%s:%s' % (r['ip'], str(r['port']))
                print >>sys.stderr, '%s\t%s\t%s' % (r['peer id'], addr, 'LEECHER')
            for seeder in swarm[0][1].keys():
                r = bdecode(swarm[0][1][seeder].bencoded)
                addr = '%s:%s' % (r['ip'], str(r['port']))
                print >>sys.stderr, '%s\t%s\t%s' % (r['peer id'], addr, 'SEEDER')

    def dump_becache_to_html(self):
        
        if not self.becache:
            return '<h2>Peer statistics</h2>\nNo peer statistics. Tracker was not contacted by any peer.'
        
        bc = self.becache
        html_string = '<h2>Peer statistics</h2>\n'
        html_string += '<table border=1 cellpadding=1 cellspacing=1>\n'
        html_string += '<tr><th>Peer ID</th><th>Network Address</th><th>Peer Type</th></tr>\n'
        
        
        for infohash in bc.keys():
            swarm = bc[infohash]
            
            html_string += '<tr><th colspan=3>%s</th></tr>' % self.allowed[infohash]['name']
            
            for leecher in swarm[0][0].keys():
                r = bdecode(swarm[0][0][leecher].bencoded)
                addr = '%s:%s' % (r['ip'], str(r['port']))
                html_string += '<tr><td>%s</td><td>%s</td><td>%s</td></tr>\n' % (r['peer id'], addr, "LEECHER")
            
            for seeder in swarm[0][1].keys():
                r = bdecode(swarm[0][1][seeder].bencoded)
                addr = '%s:%s' % (r['ip'], str(r['port']))
                html_string += '<tr><td>%s</td><td>%s</td><td>%s</td></tr>\n' % (r['peer id'], addr, "SEEDER")
...
#        for peer in self._monitor.get_monitored_peers():
#            if peer.get_peer_type() == SM.PEER_TYPE_LEECHER:
#                bc[0][0][peer.get_id()] = Bencached(bencode({'ip': peer.get_ip(), 'peer id': peer.get_id(), 'port': peer.get_port()}))
#                bc[1][0][peer.get_id()] = Bencached(bencode({'ip': peer.get_ip(), 'port': peer.get_port()}))
#                bc[2][0][peer.get_id()] = compact_peer_info(peer.get_ip(), peer.get_port())
#            else:
#                bc[0][1][peer.get_id()] = Bencached(bencode({'ip': peer.get_ip(), 'peer id' : peer.get_id(), 'port': peer.get_port()}))
#                bc[1][1][peer.get_id()] = Bencached(bencode({'ip': peer.get_ip(), 'port': peer.get_port()}))
#                bc[2][1][peer.get_id()] = compact_peer_info(peer.get_ip(), peer.get_port())
#        for supporter in self._monitor.get_monitored_supporters():
#            # supporters are always seeders
#            # supporter id has to be its peer_id for the torrent it participates in
#            ip, port = supporter.get_addr()
#            bc[0][1][supporter.get_id()] = Bencached(bencode({'ip': ip, 'peer id': supporter.get_id(), 'port': port}))
#            bc[1][1][supporter.get_id()] = Bencached(bencode({'ip': ip, 'port': port}))
#            bc[2][1][supporter.get_id()] = compact_peer_info(ip, port)
    ...
    def get_file(self, hash):
        ...
        if not self.allowed.has_key(hash):
            return (404, 'Not Found', {'Content-Type': 'text/plain', 'Pragma': 'no-cache'}, alas)
        fname = self.allowed[hash]['name'] + '.tstream'
        fpath = self.allowed[hash]['path']
        return (200, 'OK', {'Content-Type': 'application/octet-stream',
    ...
    def add_data(self, infohash, event, ip, paramslist):
        ...
        # smoothit_
        self._statistics.on_data(infohash, event, ip, paramslist)
        # _smoothit
    ...
    def get(self, connection, path, headers):
        ...
        try:
            ...
            # TRACKER EXTENSION (INTEGRATION OF SUPPORTER AND PEER MONITORING)
            if path == 'request_support':
                self._logger.info("Peer %s has requested support." % str(params('id')))
                self._monitor.received_peer_message(SM.MSG_SUPPORT_REQUIRED, params('id'))
                return (200, 'OK', {'Content-Type' : 'text/plain'}, 'Request fulfilled')
            if path == 'abort_support':
                self._logger.info("Peer %s no longer requires support." % str(params('id')))
                self._monitor.received_peer_message(SM.MSG_SUPPORT_NOT_NEEDED, params('id'))
                return (200, 'OK', {'Content-Type' : 'text/plain'}, 'Request fulfilled')
            if path == 'register_peer':
                self._monitor.register_monitored_peer(params('id'), ip, int(params('port')), SM.PEER_TYPE_LEECHER)
                self._logger.info("Registered peer at %s:%s for monitoring." % (str(ip),str(params('port'))))
                return (200, 'OK', {'Content-Type' : 'text/plain'}, 'Request fulfilled')
            if path == 'register_supporter':
                addr = (ip, int(params('port')))
                self._monitor.register_monitored_supporter(params('id'), addr, int(params('min_peer')), int(params('max_peer')))
                self._logger.info("Registered supporter at %s:%s for monitoring." % (str(ip),str(params('port'))))
                return (200, 'OK', {'Content-Type' : 'text/plain'}, 'Request fulfilled')
            #_______________________________________________________________________________________
            ...
            # data was updated, so dump becache to console
            self.dump_becache_to_console()
...
def track(args):
    ...
    # there seems to be a slight incompatibility with the trackerdefaults
    # hash and the method responsible for starting the tracker. the config
    # hash only stores key-value-pairs for keys that are defined in the
    # trackerdefaults hash (see defaults.py for details), but this method
    # expects that keys like minport, bind etc. are also present.
    # quick n dirty solution: added the necessary fields to the tracker-
    # defaults hash in default.py, so that the parsing of commandline arguments
...
#                  config['tracker_socket_timeout'], ipv6_enable = config['ipv6_enabled'])
    ...
    r.listen_forever(HTTPHandler(t.get, config['tracker_min_time_between_log_flushes']))
    t.save_state()
