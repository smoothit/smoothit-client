        ...
        try:
            ...
            if self.config['download_help']:
                ...
                if self.config['coopdl_role'] == COOPDL_ROLE_COORDINATOR:
                    from BaseLib.Core.CoopDownload.Coordinator import Coordinator
                ...
                if self.config['coopdl_role'] == COOPDL_ROLE_HELPER:
                    from BaseLib.Core.CoopDownload.Helper import Helper
...
# _2fastbt
    ...
    def _make_upload(self, connection, ratelimiter, totalup):
        return Upload(connection, ratelimiter, totalup, 
                      ...
                      self.config, is_supporter_seed = self.is_supporter_seed)#SmoothIT: supporter seed
...
#            self.upmeasure.update_rate(x)
    ...
    def startEngine(self, ratelimiter = None, vodeventfunc = None, is_supporter_seed = False, supporter_ips = []):
        self.is_supporter_seed = is_supporter_seed #SmoothIT
        ...
        self.downloader = Downloader(self.infohash, self.storagewrapper, self.picker, 
            ...
            supporter_ips = self.supporter_ips)
...
#--- 2fastbt_
        ...
        if self.helper is not None:
            from BaseLib.Core.CoopDownload.RatePredictor import ExpSmoothRatePredictor
...
# _2fastbt
	...
	#SmoothIT_ : supporter extension
    def update_support_required(self, required):
        self.downloader.send_support_required(required)
	#_SmoothIT
...
#            self.rawserver.add_task(r)
    ...
    def set_supporter_ips(self, supporter_ips):
        self._supporter_ips = supporter_ips
        print >> sys.stderr, "download_bt1: supporter ips %s " % supporter_ips
        self.downloader.supporter_ips = self.supporter_ips