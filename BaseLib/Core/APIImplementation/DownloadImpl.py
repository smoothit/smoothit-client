...
from BaseLib.Core.Utilities.unicode import metainfoname2unicode
...
class DownloadImpl:
    ...
    def __init__(self,session,tdef):
        ...
        #SmoothIT_		
        self.is_supporter_seed = session.is_supporter_seed()
        self.supporter_ips = session.get_supporter_ips()
        #_SmoothIT
    ...
    def setup(self,dcfg=None,pstate=None,initialdlstatus=None,lmcreatedcallback=None,lmvodeventcallback=None):
        ...
        try:
            ...
            if initialdlstatus == DLSTATUS_SEEDING:
                dcfg.set_max_speed(DOWNLOAD, 0.001) #very very low
    ...
    def pause(self):
        self.initialdlstatus = DLSTATUS_SEEDING
    ...
    def network_create_engine_wrapper(self,infohash,metainfo,kvconfig,multihandler,listenport,vapath,vodfileindex,lmcallback,pstate,lmvodeventcallback):
        ...
        peer_type = "-"
        try:
            peer_type = self.dlconfig['peer_type']
        except:
            pass
            
        try:                                    
            self.sd = SingleDownload(infohash,metainfo,kvconfig,multihandler,self.session.lm.get_ext_ip,listenport,vapath,vodfileindex,self.set_error,pstate,lmvodeventcallback,self.session.lm.hashcheck_done,peer_type,
                                     is_supporter_seed = self.is_supporter_seed,
                                     supporter_ips = self.supporter_ips)#SmoothIT supporter seed
            sd = self.sd
            exc = self.error
    ...
    def network_get_state(self,usercallback,getpeerlist,sessioncalling=False):
        ...
        try:
            ...
            if self.sd is None:
                ...
                #SmoothIT_ aggregate block statistics
                dlr = self.sd.dow.downloader 
                if dlr: 
                    all_stats = dlr.fetch_block_stats()
                    #print >>sys.stderr,"Block_stats = %s" % all_stats
                else:
                    all_stats = []
                #_SmoothIT
                
                ds = DownloadState(self,status,self.error,0.0,stats=stats,filepieceranges=self.filepieceranges,logmsgs=logmsgs,coopdl_helpers=coopdl_helpers,coopdl_coordinator=coopdl_coordinator,swarmcache=swarmcache, block_stats = all_stats)
